...
if(window.matchMedia("(min-width:800px)").matches) {
	...
	function useLettre(idlettre){
		...
		if(partie_en_cours.parametres.pluralite == 'non'){
			...
			var letter = _(".large-screen #"+idlettre+" .valeur").innerHTML;
...
if(document.location.href.includes("=")){
	...
	function clavier(_event_){
		if(partie_en_cours.admin){
			if(partie_en_cours.etat == 1){
				let key = checkEvent(_event_).key, spans = document.querySelectorAll(".small-screen .lettres .lettre .valeur"), elts= document.querySelectorAll(".small-screen .lettres .lettre"),letters = [], lettres = [], soluce = "";
				for(let i=0;i<spans.length;i++){
					letters[i] = {};
					letters[i].val = spans[i].innerHTML;
					lettres[i] = spans[i].innerHTML;
...
//			_(".joueur-tab a").innerHTML = "JOUEURS ("+a+")";
	...
	function useLettre(idlettre){
		...
		if(partie_en_cours.parametres.pluralite == 'non'){
			...
			var letter = _(".small-screen #"+idlettre+" .valeur").innerHTML;
...
function dicoAssocie(mot){
	let lang = "francais";
	if(tel == 1){
		lang = _(".small-screen .second-screen .langue .val").innerHTML.toLowerCase()
	}
	else{
		lang = _(".large-screen .second-screen .langue .val").innerHTML.toLowerCase();
	}
	let dico = [];
	switch(mot[0]){
		case "B":
